...
import 'package:suvandnat/app/presentation/screens/cart/widget/coupon_remove_dialog.dart';
import 'package:suvandnat/app/presentation/screens/checkout_order/widget/checkout_add_address_dialog.dart';
...
class CheckoutOrderController extends GetxController {
  ...
  final loginAddressFormKey = GlobalKey<FormState>();
  ...
  Future<void> getGuestEstimateAndShipInformationFromApi(
    ...
    try {
      ...
      log('postGuestShippingInformationResponse is:-->> $e && st is:--> $st');
...
//CountrList
  ...
  Future<void> addAddress() async {
    ...
    try {
      if (firstNameController.text != "" &&
          ...
          cityController.text != "" &&
          (stateController.text != "" || selectedState1.value.id != "") &&
  ...
  Future<void> placeOrder(CartModel? cartlist, BuildContext context) async {
    String response;
    try {
  ...
  void countryOnChanged(CountryListModel? value) {
    selectedCoutry1.value = value!;
    selectedState1.value = AvailableRegion();
    selectedStateList1.value = selectedCoutry1.value.availableRegions ?? [];
    countryName.value = value.fullNameEnglish!;
  }

  void stateOnChanged(AvailableRegion? value) {
    selectedState1.value = value!;
    stateBilling.value = selectedState1.value.name ?? '';
  }
}
