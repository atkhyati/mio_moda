...
import 'package:avoirchic/app/presentation/screens/checkout_order/widget/checkout_add_address_widget.dart';
import 'package:avoirchic/app/presentation/screens/checkout_order/widget/remove_coupon_dialog.dart';
import 'package:avoirchic/app/presentation/screens/checkout_order/widget/success_coupon_apply_dialog.dart';
...
class CheckoutOrderController extends GetxController {
  ...
  final copmpanyNameController = TextEditingController();
  ...
  Future<void> getGuestAddress() async {
    ...
    if (value.isNotEmpty) {
      ...
      phoneNumberController.text =
          checkOutOrderAddressModel.phone?.replaceAll("+", "") ?? '';
  ...
  void showDialogForSuccessfullyAppliedCoupons(
    ...
    showDialog(
      context: Get.context!,
      builder: (context) {
        return SuccessCouponApplyDialog(
  ...
  Future<void> getGuestEstimateAndShipInformationFromApi(
    ...
    debugPrint("Guest Shipping Information -> ");
    isLoading.value = true;
    try {
      String params = "";
  ...
  Future<void> getEstimateAndShipInformationFromApi() async {
    try {
      ...
      if (multiAddressModel.value.addresses?.isNotEmpty ?? false) {
        address = multiAddressModel.value.addresses?.first;
      } else {
  ...
  Future<String> postListForOrder(
    ...
    String? postCreateOrderData;

    try {
      String postCreateOrder =
          await checkoutOrderAPIRepository.postPaymentInformation(
        email: cartlist?.billingAddress?.email ?? '',
        paymentMethod: method,
        selectedAddress: selectedAddress!,
      );
      postCreateOrderData = postCreateOrder;
    ...
    return postCreateOrderData ?? '';
  }

  Future<String> postGuestOrderForOrder(
      cartlist, method, paymentId, context) async {
    isLoading.value = true;
    try {
      var podLIst = {
...
//CountrList
      ...
      if (cityController.text != "" &&
        ...
        await addressListAPIRepository.addNewAddress(
            ...
            AddAddressRequest(
                ...
                telephone:
                    "+$countryCode${phoneNumberController.value.text.toString()}",
                postcode: zipPovinceController.value.text.toString(),
                city: cityController.value.text.toString(),
                defaultBilling: false,
                defaultShipping: false,
                company: copmpanyNameController.value.text.toString()));
  ...
  void showAddDialog(BuildContext context) {
    showDialog(
      ...
      builder: (BuildContext context) {
        return CheckOutAddAddressWidget();
      },
    );
  }
  ...
  Future<void> shippingValidationAddress() async {
    ...
    debugPrint('add1 $add1');
    debugPrint('add2 $add2');
    debugPrint('add3 $add3');
    ...
    if (firstName.isNotEmpty &&
        ...
        zipCode.isNotEmpty &&
        phone.isNotEmpty) {
      debugPrint('shippingValidationAddress');
