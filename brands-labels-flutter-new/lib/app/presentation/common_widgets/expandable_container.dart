...
class _ExpandableControllerNotifier extends InheritedNotifier<ExpandableController> {
  const _ExpandableControllerNotifier({required ExpandableController? controller, required Widget child})
...
class _ExpandableThemeNotifier extends InheritedWidget {
  ...
  const _ExpandableThemeNotifier({required this.themeData, required Widget child}) : super(child: child);
...
class ExpandableIconState extends State<ExpandableIcon> with SingleTickerProviderStateMixin {
  ...
  void _expandedStateChanged() {
    ...
    } else if (!(controller?.expanded ?? false) &&
    ...
    return Padding(
      ...
      child: AnimatedBuilder(
        ...
        builder: (context, child) {
          final showSecondIcon = theme.collapseIcon! != theme.expandIcon! && (animationController?.value ?? 0) >= 0.5;
          return Transform.rotate(
            angle: (theme.iconRotationAngle ?? 0) *
                (showSecondIcon ? -(1.0 - (animationController?.value ?? 0)) : (animationController?.value ?? 0)),
