...
class _IntlPhoneFieldState extends State<IntlPhoneField> {
  ...
  void initState() {
    ...
    _countryList = widget.countries == null
        ...
        : countries
            .where((country) =>
                (widget.countries?.contains(country.code) ?? false))
            .toList();
    ...
    if (widget.autovalidateMode == AutovalidateMode.always) {
      ...
      if (value is String) {
        ...
        (value as Future<String>).then((String msg) {
  ...
  Widget build(BuildContext context) {
    return TextFormField(
      ...
      validator: (value) {
        String? error = Validators.validateMobile(
          value?.trim() ?? '',
        );
        if (widget.isError != null) {
          widget.isError!(error?.isNotEmpty??false);
        }
