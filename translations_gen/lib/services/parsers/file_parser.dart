...
abstract class FileParser {
  ...
  List<String> get supportedLanguages {
    if (parsedContents.isEmpty) {
      print('Error! File contents have not been parsed.');
    }
  ...
  List<String> getColumn(int index) =>
      parsedContents.sublist(_numberHeaderLines)
          .map((row) => row[index])
          .toList(growable: false);
  ...
  List<String> getValues(String language) {
    if (!supportedLanguages.contains(language)) {
      print('Error! Language $language is not part of parsed contents.');
    }
