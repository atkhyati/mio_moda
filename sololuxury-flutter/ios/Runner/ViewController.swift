...
class ViewController: UIViewController {
    ...
    private var paymentData: Dictionary<String,Any> = [:]
    ...
    func setPaymentData(paymentData: Dictionary<String,Any>) {
    ...
    private func makeAdyenContext() -> AdyenContext? {
        ...
        let amount = Amount(value: Decimal(Double((paymentData["amount"] as! String) ) ?? 0.0), currencyCode: (paymentData["currency"] as! String) )
        // Create the payment object with the amount and country code.
        let payment = Payment(amount: amount, countryCode: (paymentData["countryCode"] as! String) )
...
extension ViewController: AdyenSessionDelegate {
...
extension ViewController {
    private func createPaymentSessionAPI(completionHandler: @escaping (CreatePaymentSessionResponseRepresentation?) -> Void) {
        ...
        let json: [String: Any] = ["merchantAccount": AppDelegate.merchantAccount,
                                   ...
                                   "reference": "iOS Checkout Shopper",
                                   "countryCode": "IN",
                                   "lineItems": []]
